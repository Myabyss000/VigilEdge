...
start "Demo Website" cmd /k "cd /d "%VIGILEDGE_DIR%\vulnerable-app" && "%PYTHON%" app.py"
timeout /t 3 /nobreak >nul

echo [4/4] Starting WAF Dashboard...
start "VigilEdge WAF" cmd /k "cd /d "%VIGILEDGE_DIR%\waf" && set "UPSTREAM_USE_DEMO_TARGET=true" && set "UPSTREAM_DEMO_TARGET_URL=http://localhost:8080" && "%PYTHON%" -m uvicorn app:app --host 0.0.0.0 --port 5000 --reload"
timeout /t 5 /nobreak >nul
